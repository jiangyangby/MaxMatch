...
from models import get_maxmatch_model
...
parser.add_argument('--K', default=3, type=int, metavar='K',
                    help='number of strong augmented variants')  # maxmatch
...
parser.add_argument('--modeldir', default='checkpt', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model_func = get_maxmatch_model(args.arch)
    ...
    if args.trainindex_x is not None and args.trainindex_u is not None:
        ...
        train_dataset_x, train_dataset_u, val_dataset = get_imagenet_ssl_random(
            args.data, args.anno_percent, weak_type=args.weak_type, strong_type=args.strong_type, K=args.K)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, args.modeldir, 'ckpt-{}.pth.tar'.format(epoch + 1))
...
def train(train_loader_x, train_loader_u, model, optimizer, epoch, args):
    ...
    for i, (images_u, targets_u) in enumerate(train_loader_u):
        ...
        randK = random.choices(range(3), weights=(0.5, 0.3, 0.2))[0] + 1  # maxmatch
        images_u_s = torch.cat(images_u_s[:randK])  # maxmatch
        ...
        logits_u_s_K = logits_u_s.chunk(randK)  # maxmatch
        # pseudo label
        ...
        loss_u_K = torch.stack([F.cross_entropy(
            logits_u_s_K[i], pseudo_targets_u, reduction='none') for i in range(randK)], 0)  # maxmatch: (K, bs, xxx) -> (K, bs)
        loss_u_K = torch.max(loss_u_K, 0)[0]  # maxmatch: (K, bs) -> (bs, )
        loss_u = (loss_u_K * mask).mean()  # maxmatch
...
def get_imagenet_ssl(image_root, trainindex_x, trainindex_u,
    ...
    transform_u = data_transforms.MultiCropsTransform(weak_transform, strong_transform, K)  # maxmatch
...
def get_imagenet_ssl_random(image_root, percent, train_type='DefaultTrain',
                            val_type='DefaultVal', weak_type='DefaultTrain', strong_type='RandAugment', K=1):
    traindir = os.path.join(image_root, 'train')
    valdir = os.path.join(image_root, 'val')
    transform_x = data_transforms.get_transforms(train_type)
    weak_transform = data_transforms.get_transforms(weak_type)
    strong_transform = data_transforms.get_transforms(strong_type)
    transform_u = data_transforms.MultiCropsTransform(
        weak_transform, strong_transform, K)  # maxmatch
    transform_val = data_transforms.get_transforms(val_type)
...
def save_checkpoint(state, is_best, path, filename='checkpoint.pth.tar'):
    ...
    if state['epoch'] % 10 == 0:
        to_filepath = os.path.join(path, filename)
        shutil.copyfile(filepath, to_filepath)
    if is_best:
        to_filepath = os.path.join(path, 'model_best.pth.tar')
        shutil.copyfile(filepath, to_filepath)
